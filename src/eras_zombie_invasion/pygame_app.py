...
class GameApp:
    ...
    def _load_assets(self) -> dict[str, pygame.Surface]:
        ...
        mapping = {
            ...
            "soldier": "soldier.ppm",
            "zombie": "zombie.ppm",
            "barracks": "barracks.ppm",
            "tower": "tower.ppm",
            "base": "base.ppm",
            "gold": "gold_node.ppm",
            "lumber": "lumber_node.ppm",
        }
    ...
    def _draw_map(self) -> None:
        for node in self.session.nodes:
            sprite_key = "gold" if node.kind == "gold" else "lumber"
            if sprite_key in self.assets:
                self._blit_sprite(sprite_key, (node.x, node.y), 18)
            else:
        ...
        for idx, nation in enumerate(self.session.nations):
            ...
            if "base" in self.assets:
                self._blit_sprite("base", (base_x, base_y), 36)
            else:
            ...
            for building in nation.buildings:
                if building.building_type == "barracks":
                    if "barracks" in self.assets:
                        self._blit_sprite("barracks", (building.x, building.y), 26)
                    else:
                    ...
                    if "tower" in self.assets:
                        self._blit_sprite("tower", (building.x, building.y), 24)
                    else:
                        pygame.draw.circle(
                            self.screen,
                            COLOR_TOWER,
            ...
            for unit in nation.units:
                ...
                sprite_key = unit.unit_type
                if sprite_key in self.assets:
                    size = 18 if unit.unit_type == "worker" else 20
                    self._blit_sprite(sprite_key, (unit.x, unit.y), size)
                else:
        ...
        for zombie in self.session.zombies:
            if "zombie" in self.assets:
                self._blit_sprite("zombie", (zombie.x, zombie.y), 20)
            else:
    ...
    def _blit_sprite(self, key: str, pos: tuple[float, float], size: int) -> None:
